...
struct AnnotateFunctions: public PassInfoMixin<AnnotateFunctions> {
    ...
    void getGlobalFunctionName(Module& M, Function& F, LLVMContext& ctx) {
        ...
        if (!FnNameGlobal) {
            ...
            Constant *funcNameConst = ConstantDataArray::getString(ctx, F.getName(), true);
    ...
    void emitFuncArg(Function *F, Value *arg, Instruction* insertion_before) {
        ...
        FunctionType* argFnTy = FunctionType::get(
            Type::getVoidTy(ctx),
            { arg_type, PointerType::get(ctx, 0) },
            false
        );
        
        FunctionCallee resolve_arg_callee = M->getOrInsertFunction(argument_str, argFnTy);
        builder.CreateCall(resolve_arg_callee, {arg, FuncNames[F]} );
    }

    void emitFuncRetValue(Function *F, Value *retval, Instruction* insert_before) {
        ...
        if(!retval) {
            ...
            FunctionCallee resolve_ret_callee = M->getOrInsertFunction(resolve_ret, ResolveVoidFuncTy);
            ...
            FunctionType* ResolveVoidFuncTy = FunctionType::get(
                Type::getVoidTy(ctx),
                { retval->getType(), PointerType::get(ctx, 0) },
                false
            );
    ...
    void enumBasicBlock(Function *F, int64_t counter, Instruction* insert_before) {
        Module *M = F->getParent();
        LLVMContext& ctx = M->getContext();
        ConstantInt* BB_count = ConstantInt::get(IntegerType::get(ctx, 64), counter);
        ...
        FunctionCallee resolve_bb_callee = M->getOrInsertFunction(resolve_bb_str, BBCountFnType);
        ...
        for(auto &BB : F) {
            for(auto& inst : BB) {
                if (auto* retInst = dyn_cast<ReturnInst>(&inst)) {
                    Value* retVal = retInst->getNumOperands() == 1 ? retInst->getOperand(0) : nullptr;
...
//     if(!FnNameGlobal) {
//         std::string GlobalName = "resolve_fn_" + F.getName().str();
//         Constant *strConstant = ConstantDataArray::getString(ctx, F.getName(), true);
//         GlobalVariable *GV = new GlobalVariable(
//                 M,
//                 strConstant->getType(),
//                 true,
...
// void emitLavaArg(Value* v, IRBuilder<> &B, Module& M, LLVMContext& ctx, Function* F) {
//     /* Function builds the libresolve_arg_ function call with the llvm type of the argument */
    
//     /* Get the type of the argument */
//     Type* lava_arg_type = v->getType();
    
//     /* Concatenate "libresolve_arg_" prefix with llvm type of the argument */
//     std::string lava_arg_str = "libresolve_arg_" + getLLVMType(lava_arg_type);
...
//     FunctionCallee lava_arg_callee = M.getOrInsertFunction(lava_arg_str, argFnTy);
...
// void emitLavaRetVal(Value* v, IRBuilder<> &B, Module& M, LLVMContext& ctx, Function* F) {
...
//                 false
//         );
//         FunctionCallee lava_ret_callee = M.getOrInsertFunction(lava_ret_str, voidRetFnTy);
//         B.CreateCall(lava_ret_callee, { FuncNames[F] });

//     } else { /* Handles non-void case */
//         Type* lava_ret_type = v->getType();
//         std::string lava_ret_str = "libresolve_ret_" + getLLVMType(lava_ret_type);

//         /* Declare function type with arg type and pointer type for function name */
//         FunctionType *retFnTy = FunctionType::get(
//             Type::getVoidTy(ctx),
...
//         /* Build the llvm function call instruction */
//         FunctionCallee lava_arg_callee = M.getOrInsertFunction(lava_ret_str, retFnTy);
...
// void basicblockEnum(BasicBlock &BB, IRBuilder<> &B, Module& M, LLVMContext& ctx, int64_t counter, Function * F) {
//     /* Create a new LLVM obj that stores */
//     ConstantInt* BB_index = ConstantInt::get(IntegerType::get(ctx, 64), counter);
...
//     FunctionCallee lava_bb_callee = M.getOrInsertFunction(lava_bb_inst_str, voidFnTy);
//     B.CreateCall(lava_bb_callee, { BB_index, FuncNames[F] });
...
// PreservedAnalyses AnnotateFunctions::run(Module &M, ModuleAnalysisManager &MAM) {
//     LLVMContext& ctx = M.getContext();
...
//         /* Initialize a builder object to insert special libresolve instruction */
...
//             basicblockEnum(*BB, bb_builder, M, ctx, BB_index++, BB->getParent());

//             if (auto* lava_ret_inst = dyn_cast<ReturnInst>(BB->getTerminator())) {
//                 IRBuilder<> rb(lava_ret_inst);
//                 emitLavaRetVal(lava_ret_inst->getReturnValue(), rb, M, ctx, &F);
//             }
//         }
//     }
