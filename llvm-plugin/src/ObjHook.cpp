...
struct ObjHook : public PassInfoMixin<ObjHook> {
    public:
        Function *getOrCreateResolveMalloc(Module &M) {
            ...
            FunctionType *resolveMallocFuncTy = FunctionType::get(ptr_ty, { Type::getInt64Ty(Ctx)},
            false);
        ...
        Function *getOrCreateResolveFree(Module &M) {
            ...
            FunctionType *resolveFreeFuncTy = FunctionType::get(void_ty, { ptr_ty },
            false);
        ...
        Function *getOrCreateResolveCalloc(Module &M) {
            ...
            FunctionType *resolveCallocFuncTy = FunctionType::get(ptr_ty, { i64_ty, i64_ty},
            false);
            ...
            Value *realCalloc = builder.CreateCall(runtimeCalloc, { numElems, sizeElems });
        ...
        Function *getOrCreateResolveRealloc(Module &M) {
            ...
            FunctionType *resolveReallocFuncTy = FunctionType::get(ptr_ty, { ptr_ty, i64_ty },
            false);

            if (Function *F = M.getFunction("resolve_realloc"))
                return F;

            Function *resolveReallocFunc = Function::Create(
                resolveReallocFuncTy,
                GlobalValue::WeakAnyLinkage,
                "resolve_realloc",
                M
            );
            ...
            Value *realRealloc = builder.CreateCall(runtimeRealloc, { ptrArg, sizeElems });
        ...
        Function *getOrCreateResolveStrdup(Module &M) {
            ...
            FunctionType *resolveStrdupFuncTy = FunctionType::get(ptr_ty, { ptr_ty }, 
            false);
        ...
        Function* getOrCreateResolveStrndup(Module &M) {
            ...
            FunctionType *resolveStrndupFuncTy = FunctionType::get(ptr_ty, { ptr_ty, i64_ty },
            false);
            ...
            Function *resolveStrndupFunc = Function::Create(
                resolveStrndupFuncTy,
                GlobalValue::WeakAnyLinkage,
                "resolve_strndup",
                M
            );
            ...
            Value *realStrndup = builder.CreateCall(runtimeStrndup, { ptrArg, sizeElems });
        ...
        void runOnFunction(Function &F, Module& M, LLVMContext &Ctx) {
            ...
            // "resolve_calloc",
            // FunctionType::get(PointerType::get(Ctx, 0), { Type::getInt64Ty(Ctx), Type::getInt64Ty(Ctx)},
            // false ));

            // FunctionCallee resolvereallocFunc = M.getOrInsertFunction(
            // "resolve_realloc",
            // FunctionType::get(PointerType::get(Ctx, 0), { PointerType::get(Ctx, 0), Type::getInt64Ty(Ctx)},
            // false));
            ...
            for (auto* mallocCall : mallocCalls) {
                ...
                CallInst *resolve_malloc_call = builder.CreateCall(resolvemallocFunc, { size_arg });
            ...
            for (auto* freeCall : freeCalls) {
                ...
                CallInst* resolve_free_call = builder.CreateCall(resolvefreeFunc, { ptr_arg });
            ...
            for (auto* callocCall : callocCalls) {
                ...
                CallInst *resolve_calloc_call = builder.CreateCall(resolvecallocFunc, { num_elems, size_elems });
            ...
            for (auto* reallocCall : reallocCalls) {
                ...
                CallInst *resolve_realloc_call = builder.CreateCall(resolvereallocFunc, { ptr_arg, size_elems });
            ...
            for (auto* strdupCall: strdupCalls) {
                ...
                CallInst *resolve_strdup_call = builder.CreateCall(resolvestrdupFunc, { ptr_arg });
            ...
            for (auto* strndupCall: strndupCalls) {
                ...
                CallInst *resolve_strndup_call = builder.CreateCall(resolvestrndupFunc, { ptr_arg, size_arg });
