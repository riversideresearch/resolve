...
static Function *getOrCreateBoundsCheckLoadSanitizer(Module *M, LLVMContext &Ctx, Type *ty) {
    ...
    FunctionType* SanitizeLoadFuncType = FunctionType::get(ty, { ptr_ty, ptr_ty }, false);
    Function *SanitizeLoadFunc = Function::Create(SanitizeLoadFuncType, Function::InternalLinkage, handlerName, M);

    // Basic blocks 
    BasicBlock *Entry = BasicBlock::Create(Ctx, "entry", SanitizeLoadFunc);
    BasicBlock *SanitizeBlock = BasicBlock::Create(Ctx, "sanitize_block", SanitizeLoadFunc);
    BasicBlock *LoadBlock = BasicBlock::Create(Ctx, "load_block", SanitizeLoadFunc);
    BasicBlock *CheckDerivedPtrBlock = BasicBlock::Create(Ctx, "check_derived_alloc", SanitizeLoadFunc);
    BasicBlock *HeapBlock = BasicBlock::Create(Ctx, "heap_alloc_case", SanitizeLoadFunc);
    BasicBlock *StackBlock = BasicBlock::Create(Ctx, "stack_alloc_case", SanitizeLoadFunc);
    ...
    auto base_ptr_check = Builder.CreateCall(getOrCreateIsHeap(M, Ctx), { base_ptr });
    Builder.CreateCondBr(base_ptr_check, HeapBlock, CheckDerivedPtrBlock);


    Builder.SetInsertPoint(CheckDerivedPtrBlock);
    
    auto derived_ptr_check = Builder.CreateCall(getOrCreateIsHeap(M, Ctx), { derived_ptr });
    ...
    FunctionCallee BoundsCheckFunc = M->getOrInsertFunction("resolve_check_bounds", CheckBoundsFuncType);
    Value* BoundsValue = Builder.CreateCall(BoundsCheckFunc, { base_ptr, derived_ptr, ConstantExpr::getSizeOf(ty) });
    ...
    FunctionType* LogSanitizeFuncType = FunctionType::get(
        Type::getVoidTy(Ctx), 
        { ptr_ty },
        false
    );
    FunctionCallee LogSanitizeFunc = M->getOrInsertFunction("resolve_report_sanitize_mem_inst_triggered", LogSanitizeFuncType);
...
static Function *getOrCreateBoundsCheckStoreSanitizer(Module *M, LLVMContext &Ctx, Type *ty) {
    ...
    FunctionType *SanitizeStoreFuncType = FunctionType::get(Type::getVoidTy(Ctx), {ptr_ty, ptr_ty, ty}, false);
    Function *SanitizeStoreFunc = Function::Create(SanitizeStoreFuncType, Function::InternalLinkage, handlerName, M);

    BasicBlock *Entry = BasicBlock::Create(Ctx, "entry", SanitizeStoreFunc);
    BasicBlock *SanitizeBlock = BasicBlock::Create(Ctx, "sanitize_block", SanitizeStoreFunc);
    BasicBlock *StoreBlock = BasicBlock::Create(Ctx, "store_block", SanitizeStoreFunc);
    BasicBlock *CheckDerivedPtrBlock = BasicBlock::Create(Ctx, "check_derived_alloc", SanitizeStoreFunc);
    BasicBlock *HeapBlock = BasicBlock::Create(Ctx, "heap_alloc_case", SanitizeStoreFunc);
    BasicBlock *StackBlock = BasicBlock::Create(Ctx, "stack_alloc_case", SanitizeStoreFunc);
...
static Function* getOrCreateBoundsCheckMemcpySanitizer(Module *M) {
    ...
    FunctionType* ResolveCheckBoundsFuncType = FunctionType::get(
        Type::getInt1Ty(Ctx),
        { ptr_ty, ptr_ty, size_ty },
        false
    );
    FunctionCallee ResolveCheckBoundsFunc = M->getOrInsertFunction("resolve_check_bounds", ResolveCheckBoundsFuncType);

    Function *SanitizeMemcpyFunc = Function::Create(SanitizeMemcpyFuncType,
    Function::InternalLinkage, "resolve_sanitize_memcpy", M);

    BasicBlock *EntryBB = BasicBlock::Create(Ctx, "entry", SanitizeMemcpyFunc);
    BasicBlock *RemedBB = BasicBlock::Create(Ctx, "sanitize_memcpy", SanitizeMemcpyFunc);
    BasicBlock *ContExecBB = BasicBlock::Create(Ctx, "normal_exe", SanitizeMemcpyFunc);
    ...
    Value* check_src_bd = builder.CreateCall(ResolveCheckBoundsFunc, { root_src, src_ptr, size_arg });
    Value* check_dst_bd = builder.CreateCall(ResolveCheckBoundsFunc, { root_dst, dst_ptr, size_arg });
    ...
    FunctionCallee memcpy_func = M->getOrInsertFunction("memcpy", 
    FunctionType::get(ptr_ty, { ptr_ty, ptr_ty, size_ty}, false));
    Value* memcpy_ptr = builder.CreateCall(memcpy_func, { dst_ptr, src_ptr, size_arg });
...
static Value *findPtrBase(Function *F, Value *V, MemorySSA &MSSA, MemorySSAWalker *Walker) {
    ...
    auto FollowMemoryUse = [&](Instruction*I) {
        ...
        if (!I->mayReadFromMemory()) return false;
        // TODO: When does this condition not hold
        if (!MSSA.getMemoryAccess(I)) return false;
    ...
    while (!wl.empty()) {
        ...
        // Handle PHI Nodes
            ...
            // TODO Handle loops? Can we ignore blocks that are not dominating
            wl.push_unique_range(Inst->incoming_values().begin(), Inst->incoming_values().end());
...
void sanitizeLoadStore(Function *f, ModuleAnalysisManager &MAM) {
    ...
    auto &FAMProxy = MAM.getResult<FunctionAnalysisManagerModuleProxy>(*f->getParent());
    ...
    // %result = call i32 @resolve_sanitize_bounds_ld_i32(ptr %base, %ptr mem_access)
    ...
    for (auto Inst : storeList) {
        ...
        auto storeFn = getOrCreateBoundsCheckStoreSanitizer(f->getParent(), f->getContext(), valueTy);
        // Nothing to sanitize
        if (ptr == ptrBase[ptr]) continue;

        auto sanitized_store = builder.CreateCall(storeFn, {ptrBase[ptr], ptr, Inst->getValueOperand() });
        Inst->removeFromParent();
        Inst->deleteValue();
    }
}
...
void sanitizeMemcpy(Function *f, ModuleAnalysisManager &MAM) {
    ...
    for (auto Inst : memcpyList) {
        ...
        // auto sanitized_memcpy = builder.CreateCall(memcpyFn, { dst_ptr, ptrBase[dst_ptr], src_ptr, ptrBase[src_ptr], size_arg });
        auto sanitized_memcpy = builder.CreateCall(memcpyFn, { dst_ptr, dst_ptr, src_ptr, src_ptr, size_arg });
