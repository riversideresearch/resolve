...
static void getGlobalFacts(GlobalVariable &G) {
    ...
    facts.addNodeProp(G, "linkage", (G.hasExternalLinkage() ? "ExternalLinkage" : "Other"));
...
static void getFunctionFacts(Function &F) {
    ...
    facts.addNodeProp(F, "linkage", (F.hasExternalLinkage() ? "ExternalLinkage" : "Other"));
    ...
    for (BasicBlock &BB : F) {
        ...
        facts.addNodeProp(BB, "idx", std::to_string(LLVMFacts::getIndexInParent(BB)));
        if (BB.hasName())
        ...
        for (Instruction &I : BB) {
            ...
            if (auto *CB = dyn_cast<CallBase>(&I)) {
                ...
                facts.addNodeProp(I, "function_type", typeToString(*CB->getFunctionType()));
            }
        }
    }
}
...
static void embedFacts(Module &M) {            
    ...
    auto embedFactsSection = [&](StringRef sectionName, const std::string &facts) {
        ArrayRef<uint8_t> inputData(reinterpret_cast<const uint8_t*>(facts.data()), facts.size());
        ...
        GlobalVariable *gv = new GlobalVariable(M,
                                                  ...
                                                  dataArr,
                                                  "resolve" + std::string(sectionName));
