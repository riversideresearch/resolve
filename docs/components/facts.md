...
These facts are compressed with zstd and stored inside a custom ELF section in the compiled binary called `.facts`. Reachability analysis can be performed by the [reach](/components/reach) tool, which consumes these facts in its analysis. 
