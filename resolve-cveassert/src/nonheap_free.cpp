...
Function *getOrCreateResolveIsHeap(Function *F) {
  auto *M = F->getParent();
  ...
  Function *cveIsHeapFn = getOrCreateResolveHelper(M, "__cve_is_heap", cveIsHeapFnTy); 

  if (!cveIsHeapFn->empty()) { return cveIsHeapFn; }
  ...
  auto read_sp_asm = InlineAsm::get(asmTy, "mov %rsp, $0", "=r,~{dirflag},~{fpsr},~{flags}", true);
...
Function *getOrCreateFreeOfNonHeapSanitizer(
    ...
    BasicBlock *EntryBB =
        BasicBlock::Create(Ctx, "entry", cveFreeNonHeapFn);
