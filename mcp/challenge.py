...
@mcp.tool()
def build_challenge_instrumented(cwe_id: int, target_function_name: str):
    """Builds the challenge problem using the resolve remediation engine, which will remediate a CWE automatically based on the ID. Specify the function name you wish to remediate without return type or parameters."""
    ...
    for cmd in CHALLENGE_META["commands"]["build"]:
        if "docker compose build" in cmd:
            cmd = f"{cmd} --build-arg RESOLVE_LABEL_CVE=/challenge/resolve_vulnerabilities.json"
