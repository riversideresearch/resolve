...
Given a CVE description in structured `.json`, the `CVEAssert` pass inserts runtime checks to an affected function. `CVEAssert` currently supports a number of memory and arithmetic vulnerability classes.
...
A Python wrapper, `reach-wrapper.py`, provides a convenient command-line interface to interact with `reach`. For more information about `reach`, see the [`reach`](https://github.com/riversideresearch/resolve/tree/main/reach) documentation.
