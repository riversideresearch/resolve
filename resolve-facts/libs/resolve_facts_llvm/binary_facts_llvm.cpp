...
std::string resolve::binaryTypeToString(const Type &type) {
  std::string str;
...
void resolve::getBinaryGlobalFacts(BinaryLLVMFacts &facts, GlobalVariable &G) {
  facts.addNode(G);
...
void resolve::getBinaryFunctionFacts(BinaryLLVMFacts &facts, Function &F) {
  facts.addNode(F);
  facts.setName(F, F.getName());
  facts.setLinkage(F, F.hasExternalLinkage() ? Linkage::ExternalLinkage
                                             : Linkage::Other);
  facts.setFunctionType(F, binaryTypeToString(*F.getFunctionType()));
  ...
  for (BasicBlock &BB : F) {
    ...
    facts.setIdx(BB, BinaryLLVMFacts::getIndexInParent(BB));
    if (BB.hasName()) {
    ...
    for (Instruction &I : BB) {
      ...
      if (auto *CB = dyn_cast<CallBase>(&I)) {
        ...
        facts.setFunctionType(I, binaryTypeToString(*CB->getFunctionType()));
      }
    }
  }
}

void resolve::getBinaryModuleFacts(BinaryLLVMFacts &facts, Module &M) {
  facts.setSourceFile(M, M.getSourceFileName());

  for (GlobalVariable &G : M.globals()) {
    facts.addEdge(M, G, EdgeKind::Contains);

    getBinaryGlobalFacts(facts, G);
  }

  for (Function &F : M) {
    facts.addEdge(M, F, EdgeKind::Contains);

    getBinaryFunctionFacts(facts, F);
  }
}

// Embed the accumulated facts into custom ELF sections.
void resolve::embedBinaryFacts(Module &M, ArrayRef<uint8_t> facts) {
