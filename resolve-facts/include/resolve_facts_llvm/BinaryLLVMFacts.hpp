...
class BinaryLLVMFacts {
  ...
  std::unordered_map<const llvm::Module *, facts_rs::ModuleHandle>
      moduleHandles;
  std::unordered_map<const llvm::Function *, BinaryNodeId> functionIDs;
  std::unordered_map<const llvm::BasicBlock *, BinaryNodeId> basicBlockIDs;
  std::unordered_map<const llvm::Argument *, BinaryNodeId> argumentIDs;
  std::unordered_map<const llvm::Instruction *, BinaryNodeId> instructionIDs;
  std::unordered_map<const llvm::GlobalVariable *, BinaryNodeId> globalVarIDs;
  ...
  template <typename N> BinaryNodeId nodeId(const N &node) {
    return addNode(node);
  }
