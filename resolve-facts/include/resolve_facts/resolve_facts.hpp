...
struct pair_hash
    : public std::function<std::size_t(std::pair<NodeId, NodeId>)> {
  std::hash<uint64_t> hasher;
  std::size_t operator()(const std::pair<NodeId, NodeId> &k) const {
    // Hash as a single uint64_t because hashing as two individual uint32_t
    // ended up being hilariously slow which could have also just been a side
    // effect of being a poor hash combiner.
...
struct ModuleFacts {
  ...
  ModuleFacts() : nodes(), edges() {}

  std::string serialize() const;
  static ModuleFacts deserialize(std::istream &facts);
};

// Basic node ids are only unique within the context of a compilation Module
// which has its own NodeId. The full id would then be (ModuleId, NodeId)
