...
Executor::Executor(LLVMContext &ctx, const InterpreterOptions &opts,
    ...
    : Interpreter(opts), interpreterHandler(ih), searcher(0),
      ...
      replayKTest(0), replayPath(0), usingSeeds(0), distMap(0), blackList(0),
  ...
  for (std::size_t i = startIndex; i > lowestStackIndex; i--) {
    ...
    if (InvokeInst *invoke = dyn_cast<InvokeInst>(inst)) {
      ...
      if (auto *sui = dyn_cast<SearchPhaseUnwindingInformation>(ui)) {
        ...
        state.pushFrame(state.prevPC, state.pc, kf);
...
void Executor::transferToBasicBlock(BasicBlock *dst, BasicBlock *src, 
  ...
  const Instruction *instr = &dst->front();
  if (distMap && distMap->find(instr) == distMap->end()) {
    // std::cout << "stack.size() = " << state.stack.size() << std::endl;
    for (const auto& frame : state.stack) {
      if (frame.resume_pc &&
	  distMap->find(frame.resume_pc->inst) != distMap->end()) {
	goto cont;
      }
    }
    ...
    // 	frame.caller->inst->getParent()->printAsOperand(stream, false);
    // 	std::cout << "caller BB label: " << name << std::endl;
    // 	std::string callerStr;
    // 	llvm::raw_string_ostream rso(callerStr);
    // 	rso << *frame.caller->inst;
    // 	rso.flush(); // Ensure all data is written to the string
    // 	// Now, callerStr contains the pretty-printed instruction
    // 	std::cout << "caller instruction: " << callerStr << std::endl;
    //   } else {
    // 	std::cout << "no caller" << std::endl;
    //   }
    //   if (frame.resume_pc) {
    // 	std::string resumeStr;
    // 	llvm::raw_string_ostream rso(resumeStr);
    // 	rso << *frame.resume_pc->inst;
    // 	rso.flush();
    // 	std::cout << "resume instruction: " << resumeStr << std::endl;
    //   } else {
    // 	std::cout << "no resume" << std::endl;
    //   }
    // }

    terminateStateEarly(state, "Pruning state", StateTerminationType::END);
    return;
    // }
  }
 cont:
...
void Executor::terminateStateOnExecError(ExecutionState &state,
  ...
  //     std::string name;
  //     llvm::raw_string_ostream stream(name);
  //     frame.caller->inst->getParent()->getParent()->printAsOperand(stream, false);
  //     std::cout << name << std::endl;
  //   }
  // }
  // std::string name;
  // llvm::raw_string_ostream stream(name);
  // state.pc->inst->getParent()->getParent()->printAsOperand(stream, false);
  // std::cout << name << std::endl;
...
void Executor::executeAlloc(ExecutionState &state,
  ...
  if (ConstantExpr *CE = dyn_cast<ConstantExpr>(size)) {
    ...
    if (!example) {
      ...
      const auto ub = ConstantExpr::alloc(128, W);
      const auto one = ConstantExpr::alloc(1, W);
      while (example->Ugt(ub)->isTrue()) {
        ref<ConstantExpr> tmp = example->LShr(one);
        bool res;
        [[maybe_unused]] bool success =
    ...
    // Fork state in which the size is equal to zero.
    StatePair zeroSize =
      fork(state, Expr::createIsZero(size), true, BranchType::Alloc);
    if (zeroSize.first) {
      klee_message("NOTE: trying zero size malloc");
      executeAlloc(*zeroSize.first, ConstantExpr::alloc(0, W), isLocal,
                   target, zeroMemory, reallocFrom);
    }

    // StatePair fixedSize;
    // if (NullOnMallocBound) {
    //   // Fork state in which the size is above a certain bound.
    //   StatePair bigSize =
    // 	fork(*zeroSize.second,
    // 	     UgtExpr::create(size,
    // 			     ConstantExpr::alloc(std::numeric_limits<int32_t>::max() / 2, W)),
    // 	     true, BranchType::Alloc);
    //   if (bigSize.first) {
    // 	klee_message("NOTE: found malloc above bound, returning 0");
    // 	bindLocal(target, *bigSize.first, 
    // 		  ConstantExpr::alloc(0, Context::get().getPointerWidth()));
    //   }
    //   if (!bigSize.second) {
    // 	return;
    //   }
    //   fixedSize =
    // 	fork(*bigSize.second, EqExpr::create(example, size), true, BranchType::Alloc);
    // } else {
    //   fixedSize =
    // 	fork(*zeroSize.second, EqExpr::create(example, size), true, BranchType::Alloc);
    // }
